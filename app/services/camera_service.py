...
from app.api.routes import get_cameras  # Импортируем функцию прямого доступа к памяти
...
class CameraService:
    ...
    def _load_font(self):
        try:
            ...
            logger.warning(
                f"{settings.VISUALIZATION_FONT_PATH} not found, using default font. Russian characters might not display correctly.")
    ...
    def _fetch_available_cameras(self):
        # Читаем список камер напрямую из памяти вместо HTTP-запроса
        try:
            return get_cameras()
    ...
    async def _update_language(self, ws=None):
        """Notifies the backend about the language change via WebSocket."""
        lang = self.languages[self.current_lang_idx]
        logger.info(f"Switching language to: {lang}")
        # Если стрим активен, отправляем команду серверу. Если нет - язык применится при старте.
        if ws and ws.open:
            try:
                await ws.send(json.dumps({"action": "set_language", "language": lang}))
            except Exception as e:
                logger.error(f"Failed to set language via WS: {e}")
    ...
    async def run_gui_and_stream_manager(self):
        ...
        while not self.exit_service_flag:
            ...
            if current_time - last_fetch_time > 2.0:
                # Больше нет await, так как функция работает с памятью синхронно
                fetched_cameras = self._fetch_available_cameras()
                ...
                if str(fetched_cameras) != last_camera_list_str or True:
                    ...
                    y_offset += 30
                    draw.text((40, y_offset), f"Model: {settings.DETECTOR_MODEL_PATH}", font=self.font,
                              fill=(200, 200, 200))
                    y_offset += 25
                    draw.text((40, y_offset), f"Confidence: {settings.DETECTOR_CONF_THRESHOLD}", font=self.font,
                              fill=(200, 200, 200))
                    y_offset += 25
                    draw.text((40, y_offset), f"Detection WS: {settings.DETECTION_WEBSOCKET_URL}", font=self.font,
                              fill=(200, 200, 200))
                    y_offset += 25
                    ...
                    draw.text((40, y_offset), f"P. {self._get_ui_text('analyse_file')}", font=self.font,
                              fill=(100, 255, 100))
                    y_offset += 30

                    draw.text((40, y_offset + 10), f"Q. {self._get_ui_text('quit')}", font=self.font,
                              fill=(255, 100, 100))
            ...
            if key == ord('q'):
                ...
                await self._update_language(ws=None)  # Язык обновится при следующем старте стрима
    ...
    async def _run_stream_task(self, source):
        ...
        try:
            # Подключаемся с указанием текущего языка в URL
            lang = self.languages[self.current_lang_idx]
            ws_url = f"{self.detection_ws_url}?lang={lang}"
            logger.info(f"Connecting to detection WebSocket: {ws_url}")
            try:
                detection_ws = await websockets.connect(ws_url, ping_interval=10, ping_timeout=10)
            ...
            while self.running:
                ...
                if key == ord('q'):
                    ...
                    self.current_lang_idx = (self.current_lang_idx + 1) % len(self.languages)
                    await self._update_language(detection_ws)  # Отправляем язык по WebSocket
